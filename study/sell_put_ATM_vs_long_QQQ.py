...
DATA_DIR = Path("data")
OUT_DIR = Path('output')/ "atm_put_return_study"
...
opt["right"] = opt["type"].str.upper().str[0]
...
opt = opt[(opt["right"]=="P") & opt["mid_price"].notna() & opt["spot"].notna()].copy()
...
for d, S0 in spot_map.items():
    ...
    for name,(lo,hi) in BUCKETS.items():
        ...
        assigned_itm = ST < K
        below_breakeven = (ST + prem) < K
        pnl_per_share = prem - max(K - ST, 0.0)
        ret = pnl_per_share / S0

        # map stock returns for the same start date
        ...
        records.append({
            ...
            "return_dec": ret, "QQQ_ret_1d": r1, "QQQ_ret_15d": r15, "QQQ_ret_30d": r30
        })
...
# stock summaries (use the aligned days used by options)
stock_stats = []
...
for ax, b in zip(axes[0], bucket_order):
    ...
    if sub.empty: ax.set_title(f"{b} (no data)"); ax.axis("off"); continue
    ax.hist(sub["return_dec"].values, bins=60, alpha=0.9)
    ax.set_title(f"Options {b}"); ax.set_xlabel("Return (decimal)"); ax.set_ylabel("Freq"); ax.grid(True, linestyle="--", alpha=0.4)

# row2 stock
for ax, (label,col) in zip(axes[1], stock_order):
    s = results[col].dropna()
    if s.empty: ax.set_title(f"{label} (no data)"); ax.axis("off"); continue
    ax.hist(s.values, bins=60, alpha=0.9)
    ax.set_title(f"Stock {label}"); ax.set_xlabel("Return (decimal)"); ax.set_ylabel("Freq"); ax.grid(True, linestyle="--", alpha=0.4)
...
print("== OPTIONS (ATM PUT) return summary ==")
